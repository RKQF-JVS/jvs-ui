<script>
  ...
  import { nodeType, logicType, getDefaultNodeProps } from '../common/enumConst'
  ...
  export default {
    ...
    methods: {
      ...
      getDomTree (h, node) {
        ...
        if (node.type === nodeType.ROOT || node.type === nodeType.SP || node.type === nodeType.CS || node.type === nodeType.EMPTY) {
          let dom = that.getDomTree(h, node.node)
          that.addArrow(h, node, dom, true)
          return [h('div', {'class': {c: true}}, dom)]
        } else if (node.type === nodeType.CONDITION && node.conditions !== undefined && node.conditions instanceof Array) {
          let index = 0;
      ...
      addCd(node) {
        let condition = this.getConditionNode(this.getId(), node.node.id,
	        "条件" + (node.node.conditions.length), this.props, false)
        node.node.conditions.push(condition)
        // 条件排序，默认（其他情况）最后
        let temp = []
        let defaultCondi = null
        for(let i in node.node.conditions) {
          if(node.node.conditions[i].defaultCondition) {
            defaultCondi = node.node.conditions[i]
          }else{
            temp.push(node.node.conditions[i])
          }
        }
        temp.push(defaultCondi)
        this.$set(node.node, 'conditions', temp)
      ...
      addNode(type, node) {
        if (type === nodeType.TJ) {
          ...
          this.$set(node, 'node', {
            conditions: [
              this.getConditionNode(this.getId(), cdId, "条件1", this.props, false),
              this.getConditionNode(this.getId(), cdId, "其他情况", this.props, true)
            ],
            id: cdId,
            pid: node.id,
      ...
      getConditionNode (id, pid, name, props, defaultCondition) {
        return {
          groups: [//条件分组
            {
          ...
          defaultCondition: defaultCondition || false
        }
      },
